...
def passes_numeric_filter(query: str, ad_text: str) -> bool:
    query_numbers = extract_numbers(query)
    ad_numbers = extract_numbers(ad_text)

    if not query_numbers:
        return True  # ако нема бројки, semantic само

    return all(num in ad_numbers for num in query_numbers)
...
def _min_required_matches(num_terms: int) -> int:
    ...
    if num_terms <= 4:
        return num_terms - 1
    return max(3, int(num_terms * 0.6))


def search_ads(query: str, limit: Optional[int] = SEARCH_LIMIT) -> List[Dict[str, Any]]:
    """
    Keyword search vrz in-memory metadata.
    SAMO ovaa funkcija go pravi keyword search — search_db.py NE treba da postoi.
    """
    query = clean_search_prefix(query)
    terms = extract_search_terms(query)
    query_normalized = normalize_text(query)

    print(f"[DEBUG] keyword search query='{query}' terms={terms} total_ads={len(metadata)}")
...
def _make_semantic_query(query: str) -> str:
    ...
    q = re.sub(r'\b(?:dobra|dobro|dobri|sostojba|rabotna|nova|novo|novi|stara|staro)\b', '', q, flags=re.IGNORECASE)
...
def _parse_ad_price(price_str: str):
    ...
    for n in nums:
        try:
            return int(re.sub(r'\s+', '', n)), is_monthly, currency
        except ValueError:
            continue
...
def filter_by_price(
    ...
    # Ako filterot gi eliminiral site — vratiги site (ne prazen odgovor)
...
def get_search_context(
    ...
    if use_semantic:
        try:
            ...
            if is_ready():
                ...
                # Supplement so nisko threshold rezultati so keyword
    ...
    if max_price is not None:
        ...
        print(f"[DEBUG] price filter max={max_price} monthly={is_monthly} currency={query_currency} {before}→{len(current_results)}")

    if source_filter and source_filter.lower() != "all":
        current_results = [
            a for a in current_results
